...
import { withUserLock, withDrawLock } from '../utils/dbLocks.js';
...
cron.schedule('5 0 * * *', async () => {
    try {
        await runPoolDraw('daily', 0.10);
    }
    catch (e) {
        log.error('Daily draw failed', { err: e.message });
    }
});
// ─── Weekly draw ──────────────────────────────────────────────────────────────
cron.schedule('10 0 * * 1', async () => {
    try {
        await runPoolDraw('weekly', 0.60);
    }
    catch (e) {
        log.error('Weekly draw failed', { err: e.message });
    }
});
// ─── Monthly draw ─────────────────────────────────────────────────────────────
cron.schedule('15 0 1 * *', async () => {
    try {
        await runPoolDraw('monthly', 0.30);
    }
    catch (e) {
        log.error('Monthly draw failed', { err: e.message });
    }
});
// ─── Loan default detection ───────────────────────────────────────────────────
cron.schedule('0 * * * *', async () => {
    try {
        const overdue = await db.query(`SELECT id, user_id, principal FROM loans
        ...
        for (const loan of overdue.rows) {
            try {
                await withUserLock(loan.user_id, async (client) => {
                    ...
                    await client.query(`UPDATE scores SET in_default=TRUE,
             composite=GREATEST(composite-100,0)
             WHERE user_id=$1`, [loan.user_id]);
                });
                await audit({
                ...
                log.error('Default mark failed', { loanId: loan.id, err: e.message });
            }
        }
    }
    catch (e) {
        log.error('Loan default check failed', { err: e.message });
    }
});
// ─── Vault inactivity alerts ──────────────────────────────────────────────────
cron.schedule('0 */12 * * *', async () => {
    try {
        const expiring = await db.query(`SELECT v.user_id, v.inactivity_days, v.last_ping, u.fcm_token
       ...
       WHERE v.status='active'
        ...
        log.error('Vault check failed', { err: e.message });
    }
});
// ─── EMA balance updater (hourly) ─────────────────────────────────────────────
cron.schedule('30 * * * *', async () => {
    try {
        const entries = await db.query(`SELECT id, amount, avg_balance FROM pool_entries WHERE active=TRUE`);
        for (const e of entries.rows) {
            const newAvg = WeightService.updateAvgBalance(parseFloat(e.avg_balance || '0'), parseFloat(e.amount), 0.1);
            await db.query(`UPDATE pool_entries SET avg_balance=$1, last_update_at=NOW() WHERE id=$2`, [newAvg, e.id]);
        }
        log.info(`[ema] Updated ${entries.rows.length} EMAs`);
    }
    catch (e) {
        log.error('EMA updater failed', { err: e.message });
    }
});
...
cron.schedule('0 */6 * * *', async () => {
    try {
        ...
        const hv = await db.query(`SELECT referrer_id, COUNT(*) c FROM referrals
       ...
       GROUP BY referrer_id HAVING COUNT(*) > 50`);
        ...
        log.error('Fraud sweep failed', { err: e.message });
    }
});
...
cron.schedule('0 2 * * *', async () => {
    try {
        ...
        log.error('Cleanup failed', { err: e.message });
    }
});
...
cron.schedule('0 */6 * * *', async () => {
    try {
        ...
        log.error('audit.anchor failed', { err: e.message });
    }
});
...
cron.schedule('30 * * * *', async () => {
    try {
        ...
        log.error('audit.anchor.verify error', { err: e.message });
    }
});
// ─── Core draw engine (hardened with resource lock) ──────────────────────────
async function runPoolDraw(poolType, yieldShare) {
    ...
    await withDrawLock(poolType, async (client) => {
        await client.query(`UPDATE pool_draw_locks SET locked=TRUE, locked_at=NOW(), draw_at=$1 WHERE pool_type=$2`, [new Date(drawAt), poolType]);
        try {
            const rows = await client.query(`SELECT pe.user_id, pe.amount, pe.avg_balance, pe.joined_at, pe.last_update_at, pe.early_exits
         FROM pool_entries pe
         WHERE pe.pool_type=$1 AND pe.active=TRUE AND pe.prize_opted_in=TRUE`, [poolType]);
            if (!rows.rows.length) {
            ...
            const prizeAmount = parseFloat((totalDeposited * blendedApy * (days / 365) * yieldShare).toFixed(6));
            if (prizeAmount < 0.01) {
            ...
            await client.query(`INSERT INTO pool_rewards(pool_type, winner_id, amount, round_index, participants)
         VALUES($1, $2, $3, $4, $5)`, [poolType, winnerId, prizeAmount, roundIndex, entries.length]);
            await client.query(`UPDATE balances SET usdc = usdc + $1, last_updated=NOW() WHERE user_id=$2`, [prizeAmount, winnerId]);
            await client.query(`INSERT INTO transactions(user_id, type, amount, meta) VALUES($1, 'pool_win', $2, $3)`, [winnerId, prizeAmount, JSON.stringify({ pool: poolType, round: roundIndex })]);
            await audit({
            ...
            await client.query(`UPDATE pool_draw_locks SET locked=FALSE WHERE pool_type=$1`, [poolType]);
        }
    });
}
