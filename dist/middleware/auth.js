...
export function issueJwt(id, wallet) {
...
export async function generateNonce(wallet) {
    ...
    await db.query('INSERT INTO auth_nonces(wallet, nonce, expires_at) VALUES($1,$2,$3)', [wallet, nonce, expiresAt]);
    return nonce;
}
...
export async function verifySignature(wallet, signature, nonce) {
    ...
    const row = await db.query(`SELECT id FROM auth_nonces
     WHERE wallet=$1 AND nonce=$2 AND used=FALSE AND expires_at > NOW()`, [wallet, nonce]);
    if (!row.rows.length)
        return false;
