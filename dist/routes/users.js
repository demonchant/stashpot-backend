...
router.get('/me', async (req, res, next) => {
    try {
        const r = await db.query(`SELECT u.id, u.wallet, u.username, u.email, u.created_at,
              b.usdc,
              s.composite, s.tier
       FROM users u
...
router.patch('/me', validate(updateUserSchema), async (req, res, next) => {
    try {
        ...
        await db.query(`UPDATE users SET username = COALESCE($1, username), email = COALESCE($2, email) WHERE id = $3`, [username || null, email || null, req.user.id]);
        res.json({ ok: true });
    }
    catch (e) {
        next(e);
...
router.post('/fcm-token', validate(fcmTokenSchema), async (req, res, next) => {
    try {
...
router.get('/history', async (req, res, next) => {
    try {
        const r = await db.query(`SELECT type, amount, meta, created_at FROM transactions
