...
router.get('/', async (_req, res, next) => {
    try {
        const rows = await Promise.all(['daily', 'weekly', 'monthly'].map(async (pt) => {
            const r = await db.query(`SELECT COALESCE(SUM(amount),0) bal, COUNT(*) participants
         FROM pool_entries WHERE pool_type=$1 AND active=TRUE`, [pt]);
            return {
                type: pt,
...
router.get('/my-odds', async (req, res, next) => {
    try {
        const now = Date.now();
        const result = {};
        for (const pt of ['daily', 'weekly', 'monthly']) {
            const all = await db.query(`SELECT user_id, amount, avg_balance, joined_at, early_exits
         FROM pool_entries
         WHERE pool_type=$1 AND active=TRUE AND prize_opted_in=TRUE`, [pt]);
...
router.post('/deposit', requireIdempotency, // [Missing #1]
    ...
    try {
        ...
        const result = await withResourceLock(`pool:${poolId}`, async () => {
            return withUserLock(userId, async (client) => {
                ...
                const lock = await client.query('SELECT locked FROM pool_draw_locks WHERE pool_type=$1', [poolId]);
                if (lock.rows[0]?.locked) {
                    const err = new Error('Pool is locked during draw — try again in 30 seconds');
                    err.status = 423;
                    throw err;
                }
                ...
                const todays = await client.query(`SELECT COALESCE(SUM(amount), 0) AS total
             FROM transactions
             WHERE user_id=$1 AND type='deposit'
               AND created_at > NOW() - INTERVAL '24 hours'`, [userId]);
                ...
                if (alreadyToday + amount > cap) {
                    const err = new Error(`Daily deposit cap exceeded: $${alreadyToday.toFixed(2)} of $${cap} already deposited in last 24h`);
                    err.status = 429;
                    throw err;
                }
                const bal = await client.query('SELECT usdc FROM balances WHERE user_id=$1 FOR UPDATE', [userId]);
                if (!bal.rows.length || parseFloat(bal.rows[0].usdc) < amount) {
                    const err = new Error('Insufficient balance');
                    err.status = 400;
                    throw err;
                }
                await client.query('UPDATE balances SET usdc = usdc - $1, last_updated=NOW() WHERE user_id=$2', [amount, userId]);
                const existing = await client.query(`SELECT id, amount, avg_balance FROM pool_entries
             WHERE user_id=$1 AND pool_type=$2 AND active=TRUE FOR UPDATE`, [userId, poolId]);
                if (existing.rows.length) {
                    const old = existing.rows[0];
                    const newBal = parseFloat(old.amount) + amount;
                    const newAvg = WeightService.updateAvgBalance(parseFloat(old.avg_balance || old.amount), newBal);
                    await client.query(`UPDATE pool_entries SET amount=$1, avg_balance=$2, last_update_at=NOW() WHERE id=$3`, [newBal, newAvg, old.id]);
                }
                else {
                    await client.query(`INSERT INTO pool_entries(user_id, pool_type, amount, avg_balance, joined_at)
               VALUES($1, $2, $3, $3, NOW())`, [userId, poolId, amount]);
                }
                await client.query(`INSERT INTO transactions(user_id, type, amount, meta)
             VALUES($1, 'deposit', $2, $3)`, [userId, amount, JSON.stringify({ pool: poolId })]);
...
router.post('/withdraw', requireIdempotency, validate(withdrawSchema), async (req, res, next) => {
    try {
        const { poolId, amount } = req.body;
        const userId = req.user.id;
        const result = await withResourceLock(`pool:${poolId}`, async () => {
            return withUserLock(userId, async (client) => {
                const lock = await client.query('SELECT locked FROM pool_draw_locks WHERE pool_type=$1', [poolId]);
                if (lock.rows[0]?.locked) {
                    const err = new Error('Pool is locked during draw');
                    err.status = 423;
                    throw err;
                }
                const entry = await client.query(`SELECT id, amount, joined_at FROM pool_entries
             WHERE user_id=$1 AND pool_type=$2 AND active=TRUE FOR UPDATE`, [userId, poolId]);
                if (!entry.rows.length) {
                    const err = new Error('No active entry in this pool');
                    err.status = 404;
                    throw err;
                }
                const e = entry.rows[0];
                if (parseFloat(e.amount) < amount) {
                    const err = new Error('Withdrawal exceeds pool balance');
                    err.status = 400;
                    throw err;
                }
                ...
                if (newAmount < 0.01) {
                    ...
                    await client.query(`UPDATE pool_entries SET amount=$1, early_exits=early_exits+$2 WHERE id=$3`, [newAmount, isEarly ? 1 : 0, e.id]);
                }
                await client.query('UPDATE balances SET usdc = usdc + $1, last_updated=NOW() WHERE user_id=$2', [amount, userId]);
                await client.query(`INSERT INTO transactions(user_id, type, amount, meta)
             VALUES($1, 'withdraw', $2, $3)`, [userId, amount, JSON.stringify({ pool: poolId, early: isEarly })]);
...
router.get('/history', async (req, res, next) => {
    try {
        const r = await db.query(`SELECT pool_type, amount, round_index, distributed_at
       FROM pool_rewards WHERE winner_id=$1
       ORDER BY distributed_at DESC LIMIT 20`, [req.user.id]);
        res.json(r.rows);
    }
    catch (e) {
        next(e);
