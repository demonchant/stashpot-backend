...
router.get('/nonce/:wallet', nonceLimiter, validate(z.object({ wallet: solanaPubkey }), 'params'), async (req, res, next) => {
    try {
...
router.post('/verify', validate(authVerifySchema), async (req, res, next) => {
    try {
        ...
        const result = await db.query(`INSERT INTO users(wallet) VALUES($1)
         ON CONFLICT(wallet) DO UPDATE SET last_seen_at=NOW()
         RETURNING id, wallet, username, is_banned`, [wallet]);
