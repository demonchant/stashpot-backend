...
router.post('/deposit/initiate', requireIdempotency, validate(fiatDepositSchema), async (req, res, next) => {
    try {
        ...
        await db.query(`INSERT INTO fiat_transactions(user_id, provider, provider_ref, direction, fiat_amount, fiat_currency, usdc_amount)
         VALUES($1, $2, $3, 'deposit', $4, $5, $6)`, [req.user.id, provider, ref, fiat_amount, currency, usdcAmount.toFixed(6)]);
        await audit({
...
router.post('/deposit/mock-confirm', validate(mockConfirmSchema), async (req, res, next) => {
    try {
        ...
        const result = await withUserLock(req.user.id, async (client) => {
            const tx = await client.query(`SELECT * FROM fiat_transactions
           WHERE provider_ref=$1 AND user_id=$2 AND status='pending' FOR UPDATE`, [reference, req.user.id]);
            if (!tx.rows.length) {
                const err = new Error('Transaction not found or already settled');
                err.status = 404;
                throw err;
            }
            const t = tx.rows[0];
            await client.query(`UPDATE fiat_transactions SET status='completed', settled_at=NOW() WHERE provider_ref=$1`, [reference]);
            await client.query(`UPDATE balances SET usdc = usdc + $1, last_updated=NOW() WHERE user_id=$2`, [t.usdc_amount, req.user.id]);
            await client.query(`INSERT INTO transactions(user_id, type, amount, meta)
           VALUES($1, 'fiat_deposit', $2, $3)`, [req.user.id, t.usdc_amount, JSON.stringify({ ref: reference, currency: t.fiat_currency, mock: true })]);
...
router.get('/history', async (req, res, next) => {
    try {
        const r = await db.query(`SELECT provider, direction, fiat_amount, fiat_currency, usdc_amount, status, created_at
