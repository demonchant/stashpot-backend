...
function verifyHmac(secret, rawBody, signature) {
    ...
    try {
        ...
        log.warn('hmac verify failed', { err: err.message });
        return false;
    }
}
function timestampWithinWindow(ts, windowSecs = 300) {
 ...
 */
async function claimDelivery(provider, eventId, payload) {
    const r = await db.query(`INSERT INTO webhook_deliveries(provider, event_id, payload)
     VALUES($1, $2, $3)
     ON CONFLICT (provider, event_id) DO NOTHING
     RETURNING id`, [provider, eventId, JSON.stringify(payload)]);
    return r.rowCount === 1;
}
// ─── Yellow Card ──────────────────────────────────────────────────────────────
router.post('/yellow-card', rawJson, async (req, res) => {
    ...
    const timestamp = req.headers['x-yellowcard-timestamp'];
    ...
    const tx = await db.query(`SELECT * FROM fiat_transactions WHERE provider_ref = $1 AND status = 'pending'`, [reference]);
    if (!tx.rows.length) {
    ...
    try {
        await withUserLock(t.user_id, async (client) => {
            ...
            const again = await client.query(`SELECT status FROM fiat_transactions WHERE provider_ref = $1 FOR UPDATE`, [reference]);
            if (!again.rows.length || again.rows[0].status !== 'pending')
                return;
            await client.query(`UPDATE fiat_transactions SET status='completed', settled_at=NOW()
         WHERE provider_ref=$1 AND status='pending'`, [reference]);
            await client.query('UPDATE balances SET usdc = usdc + $1, last_updated=NOW() WHERE user_id=$2', [t.usdc_amount, t.user_id]);
            await client.query(`INSERT INTO transactions(user_id, type, amount, meta)
         VALUES($1, 'fiat_deposit', $2, $3)`, [t.user_id, t.usdc_amount, JSON.stringify({ provider: 'yellow_card', ref: reference })]);
        });
        await audit({
        ...
        log.error('yellow-card webhook processing failed', { err: err.message, reference });
...
router.post('/transak', rawJson, async (req, res) => {
    ...
    const timestamp = req.headers['x-transak-timestamp'];
    ...
    if (body.eventID !== 'ORDER_COMPLETED')
        return res.json({ ok: true });
    const orderId = body.webhookData?.id;
    const tx = await db.query(`SELECT * FROM fiat_transactions WHERE provider_ref = $1 AND status = 'pending'`, [orderId]);
    if (!tx.rows.length)
        return res.json({ ok: true });
    const t = tx.rows[0];
    try {
        await withUserLock(t.user_id, async (client) => {
            const again = await client.query(`SELECT status FROM fiat_transactions WHERE provider_ref = $1 FOR UPDATE`, [orderId]);
            if (!again.rows.length || again.rows[0].status !== 'pending')
                return;
            await client.query(`UPDATE fiat_transactions SET status='completed', settled_at=NOW()
         WHERE provider_ref=$1 AND status='pending'`, [orderId]);
            await client.query('UPDATE balances SET usdc = usdc + $1, last_updated=NOW() WHERE user_id=$2', [t.usdc_amount, t.user_id]);
            await client.query(`INSERT INTO transactions(user_id, type, amount, meta)
         VALUES($1, 'fiat_deposit', $2, $3)`, [t.user_id, t.usdc_amount, JSON.stringify({ provider: 'transak', ref: orderId })]);
        });
        await audit({
            action: 'fiat.deposit_completed',
            user_id: t.user_id,
        ...
        log.error('transak webhook processing failed', { err: err.message, orderId });
        return res.status(500).json({ error: 'Processing failed' });
    }
    res.json({ ok: true });
});
