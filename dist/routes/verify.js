...
import { z } from 'zod';
const router = Router();
router.get('/round/:roundId', validate(z.object({ roundId: z.string().regex(/^\d+$/).max(20) }), 'params'), validate(z.object({ pool_type: poolType.optional() }), 'query'), async (req, res, next) => {
    try {
        const { roundId } = req.params;
        const pt = req.query.pool_type || 'weekly';
        const data = await cached(`verify:round:${pt}:${roundId}`, 300, async () => {
            const draw = await db.query('SELECT * FROM pool_rewards WHERE round_index=$1 AND pool_type=$2', [roundId, pt]);
            if (!draw.rows.length)
                return null;
            const d = draw.rows[0];
            const participants = await db.query(`SELECT pe.user_id, pe.avg_balance, pe.amount, pe.joined_at, pe.early_exits, u.wallet
           FROM pool_entries pe JOIN users u ON u.id = pe.user_id
           WHERE pe.pool_type=$1 ORDER BY pe.user_id ASC`, [pt]);
            const drawAt = new Date(d.distributed_at).getTime();
            const entries = participants.rows.map((r) => ({
                userId: r.user_id,
            ...
            return {
                ...
                participants: participants.rows.map((r, i) => ({
                    wallet: r.wallet,
...
router.get('/weights/:poolType', validate(z.object({ poolType }), 'params'), async (req, res, next) => {
    try {
        const { poolType: pt } = req.params;
        const now = Date.now();
        const drawAt = now + 7 * 24 * 3_600_000;
        const rows = await db.query(`SELECT pe.user_id, pe.amount, pe.avg_balance, pe.joined_at, pe.early_exits, u.wallet
         FROM pool_entries pe JOIN users u ON u.id = pe.user_id
         WHERE pe.pool_type=$1 AND pe.active=TRUE AND pe.prize_opted_in=TRUE
         ORDER BY pe.user_id ASC`, [pt]);
        ...
        const table = WeightService.buildOddsTable(entries, drawAt);
        const totalWeight = table.reduce((s, e) => s + e.weight, 0);
        const result = rows.rows.map((r, i) => ({
            wallet: r.wallet,
            avg_balance: parseFloat(r.avg_balance || r.amount),
...
router.get('/odds/:wallet', validate(z.object({ wallet: solanaPubkey }), 'params'), async (req, res, next) => {
    try {
        ...
        for (const pt of ['daily', 'weekly', 'monthly']) {
            const pos = await db.query(`SELECT amount, avg_balance, joined_at, early_exits
           FROM pool_entries
           WHERE user_id=$1 AND pool_type=$2 AND active=TRUE`, [userId, pt]);
            if (!pos.rows.length) {
                result[pt] = { active: false };
                continue;
            }
