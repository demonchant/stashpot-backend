...
router.get('/eligible', async (req, res, next) => {
    try {
        const s = await db.query('SELECT composite, in_default FROM scores WHERE user_id=$1', [req.user.id]);
        const score = parseInt(s.rows[0]?.composite || '0', 10);
        const inDefault = s.rows[0]?.in_default || false;
        const active = await db.query(`SELECT COUNT(*) c FROM loans WHERE user_id=$1 AND status='active'`, [req.user.id]);
...
router.get('/mine', async (req, res, next) => {
    try {
        const r = await db.query(`SELECT * FROM loans WHERE user_id=$1 ORDER BY issued_at DESC`, [req.user.id]);
        res.json(r.rows);
    }
    catch (e) {
        next(e);
...
router.post('/request', requireIdempotency, validate(requestLoanSchema), async (req, res, next) => {
    try {
        ...
        const loan = await withUserLock(userId, async (client) => {
            const s = await client.query('SELECT composite, in_default FROM scores WHERE user_id=$1 FOR UPDATE', [userId]);
            const score = parseInt(s.rows[0]?.composite || '0', 10);
            if (score < p.minScore) {
                const err = new Error(`Score too low — need ${p.minScore}, have ${score}`);
                err.status = 403;
                throw err;
            }
            if (s.rows[0]?.in_default) {
                const err = new Error('In 90-day default period');
                err.status = 403;
                throw err;
            }
            const active = await client.query(`SELECT COUNT(*) c FROM loans WHERE user_id=$1 AND status='active'`, [userId]);
            if (parseInt(active.rows[0].c, 10) >= 3) {
                const err = new Error('Maximum 3 active loans');
                err.status = 400;
                throw err;
            }
            ...
            if (collateral > 0) {
                ...
                if (parseFloat(bal.rows[0]?.usdc || '0') < collateral) {
                    const err = new Error(`Need $${collateral.toFixed(2)} for collateral`);
                    err.status = 400;
                    throw err;
                }
                await client.query('UPDATE balances SET usdc = usdc - $1 WHERE user_id=$2', [collateral, userId]);
            }
            await client.query('UPDATE balances SET usdc = usdc + $1 WHERE user_id=$2', [amount, userId]);
            const inserted = await client.query(`INSERT INTO loans(user_id, loan_type, principal, apr_bps, collateral,
                             duration_days, score_at_issuance, due_at)
           VALUES($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`, [userId, loan_type, amount, p.apr, collateral, duration_days, score, dueAt]);
        ...
        await audit({
            ...
            error: e.message, ip: req.ip,
        });
        next(e);
    }
});
router.post('/:id/repay', requireIdempotency, validate(z.object({ id: z.string().uuid() }), 'params'), validate(repayLoanSchema), async (req, res, next) => {
    try {
        const { amount } = req.body;
        const userId = req.user.id;
        const loanId = req.params.id;
        const result = await withUserLock(userId, async (client) => {
            const loan = await client.query(`SELECT * FROM loans WHERE id=$1 AND user_id=$2 AND status='active' FOR UPDATE`, [loanId, userId]);
            if (!loan.rows.length) {
                const err = new Error('Loan not found or not active');
                err.status = 404;
                throw err;
            }
            const l = loan.rows[0];
            ...
            await client.query('UPDATE balances SET usdc = usdc - $1 WHERE user_id=$2', [amount, userId]);
            await client.query(`UPDATE loans SET total_repaid=$1, status=$2 WHERE id=$3`, [paid + amount, isFinal ? 'repaid' : 'active', l.id]);
            if (isFinal && parseFloat(l.collateral) > 0) {
                await client.query('UPDATE balances SET usdc = usdc + $1 WHERE user_id=$2', [l.collateral, userId]);
            }
