...
router.get('/', async (req, res, next) => {
    try {
        const r = await db.query(`SELECT v.*, COALESCE(json_agg(
...
router.post('/', requireIdempotency, validate(createVaultSchema), async (req, res, next) => {
    try {
        const { inactivity_days, beneficiaries } = req.body;
        const sum = beneficiaries.reduce((s, b) => s + b.pct, 0);
        if (sum !== 100) {
        ...
        const vaultId = await withUserLock(req.user.id, async (client) => {
            const v = await client.query(`INSERT INTO vaults(user_id, inactivity_days) VALUES($1, $2) RETURNING id`, [req.user.id, inactivity_days]);
            const id = v.rows[0].id;
            for (const b of beneficiaries) {
                await client.query(`INSERT INTO vault_beneficiaries(vault_id, wallet, pct) VALUES($1, $2, $3)`, [id, b.wallet, b.pct]);
            }
            return id;
        });
        await audit({
...
router.post('/:id/ping', validate(z.object({ id: z.string().uuid() }), 'params'), async (req, res, next) => {
    try {
        const r = await db.query(`UPDATE vaults SET last_ping=NOW()
         WHERE id=$1 AND user_id=$2 AND status='active' RETURNING id`, [req.params.id, req.user.id]);
        if (!r.rows.length)
...
router.delete('/:id', validate(z.object({ id: z.string().uuid() }), 'params'), async (req, res, next) => {
    try {
        const r = await db.query(`UPDATE vaults SET status='cancelled'
         WHERE id=$1 AND user_id=$2 AND status='active' RETURNING id`, [req.params.id, req.user.id]);
        if (!r.rows.length)
