 ...
 */
function signAnchor(chainHead, rowCount, signedAt) {
...
async function getChainHead() {
    const r = await db.query(`SELECT row_hash, COUNT(*) OVER() AS total_rows
 ...
 */
async function publishAnchor(record) {
...
export async function anchorAuditLog() {
    ...
    const last = await db.query('SELECT chain_head FROM anchor_log ORDER BY signed_at DESC LIMIT 1');
    ...
    await db.query(`INSERT INTO anchor_log(id, chain_head, row_count, signed_at, signature, destinations)
     VALUES($1, $2, $3, $4, $5, $6)`, [
        record.anchor_id,
        record.chain_head,
        record.row_count,
 ...
 */
export async function verifyAgainstLastAnchor() {
    const last = await db.query('SELECT chain_head, row_count FROM anchor_log ORDER BY signed_at DESC LIMIT 1');
    if (!last.rows.length) {
    ...
    const anchored = await db.query('SELECT 1 FROM audit_log WHERE row_hash = $1', [anchor.chain_head]);
