 ...
 */
const SCALE = 1000000000n; // 9 decimals
const LAMBDA = 150000000n; // 0.15 × SCALE
 ...
 */
function pubkeyTo32(walletBase58) {
 ...
 */
export function computeLeaf(wallet, weight, cumStart, cumEnd) {
 ...
 */
export function buildTree(participants) {
    ...
    for (const p of sorted) {
        ...
        if (w === 0n)
            continue; // Zero-weight entries are excluded — see [P4]
    ...
    const fingerprint = crypto.createHash('sha256')
        .update(leaves.map(l => `${l.wallet}|${l.weight.toString()}|${l.cumStart.toString()}|${l.cumEnd.toString()}`).join('\n'))
        .digest();
    return {
...
// ─── Proof generation ────────────────────────────────────────────────────────
export function getProof(tree, walletBase58) {
    const idx = tree.leaves.findIndex(l => l.wallet === walletBase58);
    if (idx < 0)
        throw new Error('Wallet not in tree');
    const proof = [];
 ...
 */
export function verifyTree(participants, expectedRoot) {
...
//# sourceMappingURL=MerkleBuilder.js.map