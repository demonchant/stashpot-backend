...
export async function requireIdempotency(req, res, next) {
    ...
    const cached = await db.query('SELECT response FROM idempotency_keys WHERE key = $1', [fullKey]);
    ...
    const conflict = await db.query("SELECT 1 FROM idempotency_keys WHERE key LIKE $1 AND key != $2 LIMIT 1", [`${scopePrefix}%`, fullKey]);
    ...
    res.json = ((body) => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
            db.query('INSERT INTO idempotency_keys(key, response) VALUES($1, $2) ON CONFLICT DO NOTHING', [fullKey, JSON.stringify(body)]).catch(err => console.error('[idempotency] cache write failed:', err.message));
        }
