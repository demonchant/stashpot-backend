...
export async function refreshFeatures() {
    try {
        const r = await db.query('SELECT feature, enabled FROM feature_flags WHERE feature = ANY($1::text[])', [ALL_KEYS]);
        // Start from defaults; overlay DB rows
        const next = { ...DEFAULTS };
        for (const row of r.rows) {
            if (ALL_KEYS.includes(row.feature)) {
                next[row.feature] = !!row.enabled;
            }
        }
        ...
        log.error('feature.refresh_failed', { err: err.message });
 ...
 */
export async function setFeature(key, enabled, changedBy) {
    await db.query(`INSERT INTO feature_flags(feature, enabled, changed_by, changed_at)
     VALUES ($1, $2, $3, NOW())
     ON CONFLICT (feature) DO UPDATE
     SET enabled = EXCLUDED.enabled,
         changed_by = EXCLUDED.changed_by,
         changed_at = NOW()`, [key, enabled, changedBy]);
    await db.query(`INSERT INTO feature_events(feature, enabled, changed_by) VALUES($1, $2, $3)`, [key, enabled, changedBy]);
 ...
 */
export function startFeaturePoller(intervalMs = 10_000) {
    if (pollerHandle)
        return;
...
export function stopFeaturePoller() {
    if (pollerHandle) {
...
export const FEATURES = new Proxy({}, {
    ...
    has(_target, prop) { return ALL_KEYS.includes(prop); },
